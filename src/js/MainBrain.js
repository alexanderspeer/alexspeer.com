...
class MainBrain extends AbstractApplication {
  ...
  startIntro() {
    ...
    // Show skip hint text in bottom left corner
    this.createSkipHint();
        ...
        onComplete: () => {
          ...
          const transitionTimeout = setTimeout(() => {
            ...
            const captureTimeout = setTimeout(() => {
              ...
              // Animation complete
              this.isStartupAnimationPlaying = false;
              // Hide skip hint when animation completes
              this.hideSkipHint();
  ...
  createBottomNavigationBar() {
    ...
    const socialLinks = [
      ...
      {
        name: 'Email',
        url: 'mailto:alexspeer12@gmail.com',
        svg: `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 100 100">
      ...
      {
        name: 'Bookshelf',
        url: 'https://bookshelf-hermes-4f6d58f1165f.herokuapp.com/u/alexspeer12',
        svg: `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 100 100">
      ...
      {
        name: 'Euterpe',
        url: 'https://euterpe-c0dcbd4f17ec.herokuapp.com/',
        svg: `<img src="favicons/icons8-musical-note-48.png" alt="Euterpe" style="width: 32px; height: 32px; object-fit: contain; filter: brightness(0) invert(1);" />`
      }
    ];
    ...
    socialLinks.forEach((link, index) => {
      ...
      // Check if this is the music icon (Euterpe)
      const isMusicIcon = link.name === 'Euterpe';
      const isBookIcon = link.name === 'Bookshelf';
      const isAfterBookIcon = isMusicIcon && index > 0 && socialLinks[index - 1].name === 'Bookshelf';

      iconLink.style.cssText = `
        display: inline-flex;
        align-items: center;
        justify-content: center;
        width: ${isMusicIcon ? MUSIC_ICON_SIZE : '40px'};
        height: ${isMusicIcon ? MUSIC_ICON_SIZE : '40px'};
        padding: 6px;
        border-radius: 12px;
        cursor: pointer;
        ...
        position: relative;
        ${isAfterBookIcon ? `margin-left: ${MUSIC_ICON_GAP};` : ''}
      `;

      // Update image size in the innerHTML for music icon
      let content = link.svg;
      if (isMusicIcon) {
        // Replace the width and height in the img tag
        content = content.replace(/width:\s*\d+px/g, `width: ${MUSIC_ICON_SIZE}`);
        content = content.replace(/height:\s*\d+px/g, `height: ${MUSIC_ICON_SIZE}`);
      }

      iconLink.innerHTML = content;
  ...
  createSkipHint() {
    ...
    if (document.getElementById('skip-animation-hint')) {
      return;
    }

    // Create the hint text element
    const hint = document.createElement('div');
    hint.id = 'skip-animation-hint';
    hint.textContent = 'Press ESC to skip animation';
    hint.style.cssText = `
      position: fixed;
      bottom: 20px;
      left: 20px;
  ...
  hideSkipHint() {
    const hint = document.getElementById('skip-animation-hint');
    if (hint) {
